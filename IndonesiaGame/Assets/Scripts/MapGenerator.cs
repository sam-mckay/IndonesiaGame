...
public class MapGenerator : MonoBehaviour {
    ...
    void CreateMapRegions()
    {
        ...
        {
            if (roomRegion.Count < floorThresholdSize)
            {
    ...
    List<Coord> GetRegionTiles(int startX, int startY)
    {
        ...
        frontier.Enqueue(new Coord(startX, startY));
        mapFlags[startX, startY] = 1;

        while (frontier.Count > 0)
        {
            Coord tile = frontier.Dequeue();
            tiles.Add(tile);
            ...
            for (int x = tile.tileX - 1; x <= tile.tileX + 1; x++)
            {
                for (int y = tile.tileY - 1; y <= tile.tileY + 1; y++)
                {
                    if(IsInMapRange(x,y) && (y == tile.tileY || x == tile.tileX))
                    {
                        ...
                        {
                            mapFlags[x, y] = 1;
                            frontier.Enqueue(new Coord(x, y));
                        }
                    }
                }
            }
    ...
    void RandomFillMap()
    {
        ...
        for(int x = 0; x < width; x++)
        {
            for(int y = 0; y < height; y++)
            {
                ...
                {
                    map[x, y] = (randomNumGen.Next(0, 100) < randomFillPercent) ? 1 : 0;
                }
            }
        }
    }
